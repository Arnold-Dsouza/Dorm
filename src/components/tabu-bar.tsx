...
import { Martini, Calendar, Clock, XCircle, MapPin, PartyPopper, CalendarOff, Check, Gift, Pencil, Trash2, Image as ImageIcon } from "lucide-react";
...
interface TabuBarProps {
    title?: string;
}

export default function TabuBar({ title = 'Bar' }: TabuBarProps = {}) {
    const [dorm, setDorm] = useState<ReturnType<typeof getDormConfig> | null>(null);
    const pageId = dorm?.id === 'tabu' ? 'tabuBar' : 'bar';
                 ...
                 {isAdmin && (
                    <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                        ...
                        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
                            <DialogHeader><DialogTitle>Edit {title} Details</DialogTitle></DialogHeader>
                             <div className="grid gap-4 py-4">
                                ...
                                {editableContent?.upcomingEvents?.map((event, index) => (
                                    <div key={event.id} className="p-2 border rounded-md space-y-2 relative">
                                        ...
                                        <Label>Date</Label><Input type="date" value={event.date} onChange={e => handleFieldChange('upcomingEvents', index, 'date', e.target.value)}/>
                                        <Label>Time</Label><Input type="time" value={event.time} onChange={e => handleFieldChange('upcomingEvents', index, 'time', e.target.value)}/>
